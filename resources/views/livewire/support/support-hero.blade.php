@php
$isAr = $lang === 'ar';
$title = $panner?->{"title_{$lang}"} ?? ($isAr ? 'مركز المساعدة والدعم' : 'Help & Support Center');
$image = $panner?->{"image_{$lang}"} ? storage_public_url($panner->{"image_{$lang}"}) : asset('/img/ImagesWeb/abouthero.svg');
$btn = $panner?->{"btn_{$lang}"} ?? ($isAr ? 'تواصل معنا الآن' : 'Contact Us Now');
@endphp

<div dir="{{ $isAr ? 'rtl' : 'ltr' }}" class="relative bg-gray-900 max-w-7xl mx-auto m-4 w-full p-4 overflow-hidden rounded-3xl md:p-12 lg:p-24" style="background-image: url({{ $image }}); background-size: cover; background-position: center;">
