...
$lang = app()->getLocale();
$isAr = $lang === 'ar';
$intro1 = $polices?->{"intro_{$lang}"} ?? '';
$intro2 = $polices?->{"text_{$lang}"} ?? '';
$sections = is_array($polices->jsoning_content ?? null) ? $polices->jsoning_content : [];
@endphp
        ...
        <div class="space-y-12" dir="{{ $isAr ? 'rtl' : 'ltr' }}">
            @forelse($sections as $idx => $section)
                ...
                <div class="bg-white p-8 opacity-0 translate-y-6" data-delay="{{ 100 + ($idx * 100) }}" data-animate>
                    ...
                    @endif
                    @if($sectionContent)
                        <div class="text-neutral-700 leading-relaxed space-y-4 mb-4">
                            {!! nl2br(e($sectionContent)) !!}
                        </div>
                    @endif
                    @if(count($sectionItems))
                        <ul class="list-disc list-inside text-neutral-700 space-y-2 {{ $isAr ? 'mr-4' : 'ml-4' }}">
