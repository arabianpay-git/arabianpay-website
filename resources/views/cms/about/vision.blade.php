...
@section('title', 'من نحن — الرؤية والمحتوى')
@section('heading', 'منصة تفهمك (المحتوى الرئيسي)')
@section('content')
<div class="card wide">
    <form action="{{ route('cp.about.vision.update') }}" method="post">
        @csrf @method('PUT')
        <div class="grid2">
        ...
        <div class="mb"><label>JSON (بطاقات الرؤية/الرسالة)</label><textarea name="jsoning_raw" class="code" required>{{ old('jsoning_raw', $jsoning) }}</textarea></div>
