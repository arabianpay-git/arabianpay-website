...
@section('content')
<div class="card wide">
    <form action="{{ $editing ? route('cp.support.categories.update', $category) : route('cp.support.categories.store') }}" method="post">
        ...
        <a href="{{ route('cp.support.categories.index') }}" class="muted" style="margin-inline-start:1rem;">رجوع</a>
    </form>
</div>
@endsection
